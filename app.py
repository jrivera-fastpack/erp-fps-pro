...
def main_app():
    ...
    with tab3:
        ...
        filtro_tipo = c_v2.radio("Tipo de Servicio (Filtra la vista General):", ["Todos", "SSEE", "SE TERRENO"], horizontal=True)
    ...
    with tab4:
        ...
        if not nvs_all:
            ...
            tab_global, tab_individual, tab_ocupacion, tab_historial, tab_tabla, tab_pendientes, tab_proyeccion, tab_facturados = st.tabs([
                "🌍 Global Mensual", 
                "🔍 Análisis Individual", 
                "👥 Ocupación Personal",
                ...
                if df_kpi_mes.empty and total_dias_ejecutados == 0 and total_dias_proyectados == 0:
                    ...
                    with c_graf1:
                        ...
                        fig_tiempos.add_trace(go.Bar(
                            name='Planificado (Matriz Semanal)',
                            x=['Desempeño Operativo'],
                            y=[total_dias_proyectados],
                            marker_color='#3498DB',
                            text=[f"{total_dias_proyectados} Días Planificados"],
                            textposition='outside',
                            width=0.4
                        ))
                        
                        fig_tiempos.add_trace(go.Bar(
                            name='Ejecutado Real (Carta Gantt)',
                            x=['Desempeño Operativo'],
                            y=[total_dias_ejecutados],
                            marker_color='#2ECC71',
                            text=[f"{total_dias_ejecutados} Días Reales"],
                            textposition='inside',
                        ...
                        fig_tiempos.update_layout(
                            barmode='overlay', 
                            title=f"Planificado vs Real - {mes_sel_global} {anio_sel_global}",
                            yaxis_title="Cantidad de Días-Hombre", 
                            plot_bgcolor='white',
                            legend=dict(orientation="h", yanchor="bottom", y=1.05, xanchor="center", x=0.5)
                        )
                        
                        fig_tiempos.add_hline(
                            y=capacidad_neta_mes, 
                            line_dash="dash", 
                            line_color="#95A5A6", 
                            annotation_text=f"Capacidad Máx. del Equipo ({capacidad_neta_mes} Días)", 
                    ...
                    with c_graf2:
                        st.markdown(f"**Ranking de Avance Operativo (%) - {mes_sel_global}**")
                        
                        df_ssee_chart = df_kpi_mes[df_kpi_mes['tipo_servicio'] == 'SSEE'].sort_values('Avance_%', ascending=True)
                        df_terreno_chart = df_kpi_mes[df_kpi_mes['tipo_servicio'] == 'SE TERRENO'].sort_values('Avance_%', ascending=True)
                        
                        if not df_ssee_chart.empty:
                            fig_ranking_ssee = px.bar(
                                df_ssee_chart, y="Proyecto_Label", x="Avance_%", color="Avance_%",
                                color_continuous_scale=[[0, 'red'], [0.5, 'yellow'], [1, 'green']],
                                title="🔹 SSEE (Salas Eléctricas)", text="Avance_%"
                            )
                            fig_ranking_ssee.update_traces(texttemplate='%{text:.1f}%', textposition='outside')
                            fig_ranking_ssee.update_layout(
                                xaxis_title="Avance Físico (%)", yaxis_title="", coloraxis_showscale=False, 
                                plot_bgcolor='white', height=max(200, len(df_ssee_chart)*40), margin=dict(l=10, r=20, t=40, b=10)
                            )
                            st.plotly_chart(fig_ranking_ssee, use_container_width=True)
                        else:
                            st.info("No hay proyectos SSEE (Salas Eléctricas) este mes.")
                            
                        if not df_terreno_chart.empty:
                            fig_ranking_terreno = px.bar(
                                df_terreno_chart, y="Proyecto_Label", x="Avance_%", color="Avance_%",
                                color_continuous_scale=[[0, 'red'], [0.5, 'yellow'], [1, 'green']],
                                title="🔸 SE Terreno", text="Avance_%"
                            )
                            fig_ranking_terreno.update_traces(texttemplate='%{text:.1f}%', textposition='outside')
                            fig_ranking_terreno.update_layout(
                                xaxis_title="Avance Físico (%)", yaxis_title="", coloraxis_showscale=False, 
                                plot_bgcolor='white', height=max(200, len(df_terreno_chart)*40), margin=dict(l=10, r=20, t=40, b=10)
                            )
                            st.plotly_chart(fig_ranking_terreno, use_container_width=True)
                        else:
                            st.info("No hay proyectos SE Terreno este mes.")
            ...
            with tab_individual:
                ...
                if nv_seleccionada:
                    ...
                    if asig_all_raw:
                        ...
                        df_acts_matriz = df_all_temp[(df_all_temp['id_nv'] == row_nv['id_nv']) & (df_all_temp['actividad_ssee'] == 'PROYECCION_GLOBAL')]
                        if not df_acts_matriz.empty:
                            ...
                            for _, row_act in df_acts_matriz.iterrows():
                                ...
                                while curr <= f_f:
                                    ...
                                    curr += timedelta(days=1)
                            d_p_matriz = float(len(fechas_activas_matriz))
                    ...
                    holgura_dias = d_p_matriz - d_e
                    col_m4.metric(
                        "Ejecución de Tiempos (Días)", 
                        f"{d_e:.1f} Reales / {d_p_matriz:.1f} Plan", 
                        f"{holgura_dias:.1f} Días Restantes", 
                        delta_color="inverse" if holgura_dias < 0 else "normal",
                        help="El Plan se calcula en base a lo asignado en la Matriz Semanal. Lo Real corresponde al avance en Gantt."
                    )
                    ...
                    with cg1:
                        ...
                        fig_bar_d.add_trace(go.Bar(
                            name='Planificado (Matriz Semanal)',
                            x=['Tiempos del Proyecto'],
                            y=[d_p_matriz],
                            marker_color='#3498DB',
                            text=[f"{d_p_matriz} Días Planificados"],
                            textposition='outside',
                            width=0.4
                        ))
                        
                        color_real = "#E74C3C" if d_e > d_p_matriz else "#2ECC71"
                        fig_bar_d.add_trace(go.Bar(
                            name='Real (Carta Gantt)',
                            x=['Tiempos del Proyecto'],
                            y=[d_e],
                            marker_color=color_real,
                            text=[f"{d_e} Días Reales"],
                            textposition='inside',
                            insidetextfont=dict(color='white', weight='bold'),
                            width=0.25
                        ))
            ...
            with tab_ocupacion:
                ...
                total_dias_mes = len(dias_del_mes)
                ...
                for esp in ESPECIALISTAS:
                    ...
                    for a in asigs_esp:
                        ...
                        curr = max(f_i, fecha_inicio_mes)
                        end_curr = min(f_f, fecha_fin_mes)
                        ...
                        while curr <= end_curr:
                            ...
                            if incluye_f or (not es_finde and not es_feriado):
                                if tipo_asignacion == 'AUSENCIA': 
                                    dias_ausente.add(curr)
                                elif tipo_asignacion == 'INTERNO': 
                                    dias_interno.add(curr)
                                elif tipo_asignacion == 'SSEE': 
                                    dias_ssee.add(curr)
                                elif tipo_asignacion == 'SE TERRENO': 
                                    dias_terreno.add(curr)
                            curr += timedelta(days=1)
                            
                    dias_ssee = dias_ssee - dias_ausente
                    dias_terreno = dias_terreno - dias_ausente
                    dias_interno = dias_interno - dias_ausente
                    
                    val_ssee = len(dias_ssee)
                    val_terreno = len(dias_terreno)
                    val_interno = len(dias_interno)
                    val_ausente = len(dias_ausente)
                    
                    dias_ocupados_totales = len(dias_ssee | dias_terreno | dias_interno)
                    
                    if filtro_ocup == "⚡ SSEE": 
                        dias_graficar = val_ssee
                    elif filtro_ocup == "👷 SE Terreno": 
                        dias_graficar = val_terreno
                    elif filtro_ocup == "🏢 Oficina / Interno": 
                        dias_graficar = val_interno
                    else:
                        dias_graficar = dias_ocupados_totales
                        
                    dias_disponibles = total_dias_mes - dias_graficar
                    if dias_disponibles < 0: dias_disponibles = 0
                    
                    pct_ocupacion = (dias_graficar / total_dias_mes * 100) if total_dias_mes > 0 else 0
                    
                    data_ocupacion.append({
                        "Especialista": esp,
                        "Días Trabajados (Según Filtro)": dias_graficar,
                        "Días SSEE": val_ssee,
                        "Días SE Terreno": val_terreno,
                        "Días Oficina": val_interno,
                        "Días Ausente": val_ausente,
                ...
                fig_oc = px.bar(
                    df_oc_sorted, 
                    x="Especialista", 
                    y=["Días Trabajados (Según Filtro)", "Días Disponibles"],
                    ...
                    color_discrete_map={
                        ...
                        "Días Disponibles": "#2ECC71"
                    }
                )
                fig_oc.update_layout(
                    yaxis_title=f"Cantidad de Días (Total Mes: {total_dias_mes})", 
                    plot_bgcolor='white', 
                    barmode='stack', 
            ...
            with tab_tabla:
                ...
                if not df_hitos_mes.empty:
                    ...
                    mostrar_tabla_servicio(df_ssee, "🔹 Facturación SSEE (Salas Eléctricas)")

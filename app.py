...
hola me ayudas con la moja de este codigo es una aplicacion de gestion de personal pero cual agrego una persona nueva en el una nota de vente en una semna y depesu agrego en la misma persona en otro semna me borra la carga anterior no puedo tener rotacion de personal por queno me deja registrso que ya estan caragdos en la aplicacion import streamlit as st
            ...
            with t_pa:
                ...
                if not df_pe.empty:
                    ...
                    def show_b_log(df_sub, tit):
                        ...
                        tot_usd_sub = df_sub['usd'].sum()

                        c1, c2 = st.columns(2)

                        c1.metric("Total Pendiente (USD)", f"USD ${tot_usd_sub:,.2f}")

                        c2.metric("Total Pendiente (CLP)", f"CLP ${(tot_usd_sub * tasa_cambio):,.0f}".replace(",", "."))

                        

                        df_s = df_sub[['id_nv', 'cliente', 'moneda', 'monto_vendido', 'monto_pendiente', 'estado_facturacion']].copy()

                        df_s = pd.concat([df_s, pd.DataFrame([{'id_nv': 'TOTALES', 'monto_pendiente': f"USD ${tot_usd_sub:,.2f} / CLP ${(tot_usd_sub*tasa_cambio):,.0f}".replace(",", ".") }])], ignore_index=True)
            ...
            with t_f:
                ...
                if not hf.empty: 
                    ...
                    for p in hf['Periodo'].unique():
                        ...
                        tot_usd_fact = df_per['usd'].sum()

                        

                        c1, c2 = st.columns(2)

                        c1.metric(f"Total Facturado en {p} (USD)", f"USD ${tot_usd_fact:,.2f}")

                        c2.metric(f"Total Facturado en {p} (CLP)", f"CLP ${(tot_usd_fact * tasa_cambio):,.0f}".replace(",", "."))
...
if not st.session_state.authenticated: 
    ...
    main_app() eres un especilista en programacion

¡Hola! Entiendo perfectamente el dolor de cabeza que te está causando esto. Como inteligencia artificial, analizo el código de forma lógica, y veo exactamente dónde está el "hoyo negro" que se está tragando tus registros anteriores.

El problema es un clásico caso de borrado en cascada no intencional.

🔍 El Origen del Problema
En el MÓDULO 2: MATRIZ (específicamente en la pestaña "Asignar Proyección"), tienes una línea de código que actúa como una aplanadora. Cada vez que presionas el botón "Guardar Proyección", el sistema le dice a la base de datos: "Bórgame absolutamente todos los registros de este proyecto antes de guardar los nuevos".

Aquí está la línea culpable (cerca de la línea 273 de tu código):

Python
if st.form_submit_button("Guardar Proyección", use_container_width=True):
    # ¡ESTA ES LA LÍNEA QUE BORRA TODO TU HISTORIAL!
    supabase.table("asignaciones_personal").delete().eq("id_nv", nv_data_sel['id_nv']).eq("actividad_ssee", "PROYECCION_GLOBAL").execute()
Como ya tienes una pestaña llamada "Eliminar Proyección" (tab_proy2) que sirve para borrar errores manualmente, esta línea destructiva automática no es necesaria y es la que te impide tener rotación en distintas semanas.

🛠️ La Solución
Para arreglarlo, simplemente debes eliminar o comentar esa línea para que el sistema empiece a sumar (añadir) registros nuevos en lugar de reemplazar todo el bloque.

Busca este bloque de código en tu archivo:

Código Actual (El que tiene el error):
...
if st.form_submit_button("Guardar Proyección", use_container_width=True):
    ...
    for b in bloques_gen: 
        safe_insert_asignacion({"id_nv": nv_data_sel['id_nv'], "especialista": b['especialista'], "fecha_inicio": str(b['f_ini']), "fecha_fin": str(b['f_f']), "hh_asignadas": 0, "actividad_ssee": "PROYECCION_GLOBAL", "comentarios": b['comentarios'], "progreso": 0, "hora_inicio_t": h_inicio_val.strftime('%H:%M') if h_inicio_val else '08:00', "hora_fin_t": h_fin_val.strftime('%H:%M') if h_fin_val else '17:30', "horas_diarias": 0 if b.get('es_descanso') else (h_diarias_val if h_diarias_val else 0)})
    st.success("✅ Actualizado"); st.rerun()
Código Corregido (Reemplázalo por este):

Python
if st.form_submit_button("Guardar Proyección", use_container_width=True):
    # Línea eliminada para permitir rotación y no borrar semanas pasadas.
    
    bloques_gen = generar_bloques_turno(f_ini, dias_proy, modalidad_matriz, especialistas_sel)
    for b in bloques_gen: 
        safe_insert_asignacion({
            "id_nv": nv_data_sel['id_nv'], 
            "especialista": b['especialista'], 
            "fecha_inicio": str(b['f_ini']), 
            "fecha_fin": str(b['f_f']), 
            "hh_asignadas": 0, 
            ...
            "horas_diarias": 0 if b.get('es_descanso') else (h_diarias_val if h_diarias_val else 0)
        })
    st.success("✅ Proyección agregada al historial"); st.rerun()
💡 ¿Qué pasará ahora?
Cuando agregues a "Juan" en la Semana 1, se guardará.

Cuando agregues a "Juan" en la Semana 3 para el mismo proyecto, el sistema simplemente añadirá esas nuevas fechas a la base de datos sin tocar la Semana 1.

Si cometes un error y asignas mal una semana, simplemente vas a la pestaña Eliminar Proyección y borras ese registro específico, manteniendo el resto intacto.

me das el codigo completo

¡Claro que sí! Aquí tienes el código completo y listo para copiar y pegar. Ya le he aplicado la corrección exacta en el Módulo 2 (Matriz) para que el botón de "Guardar Proyección" no borre tu historial y te permita agregar nuevas semanas sin problemas.

He ajustado la indentación a espacios normales para asegurar que Streamlit no te arroje errores de formato al ejecutarlo.
...
Los chats de Fastpack no se usan para mejorar nuestros modelos. Gemini es una IA y puede cometer errores. Tu privacidad y GeminiSe abrirá en una ventana nueva
